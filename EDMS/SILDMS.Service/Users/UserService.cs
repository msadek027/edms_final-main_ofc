...
namespace SILDMS.Service.Users
{
    public partial class UserService : IUserService
    {
        ...
        {
            return userDataService.GetIpValidationInfo(user,ip);
        }


        public ValidationResult GetAllUserMail(out List<string> mailList)
        {
            mailList = userDataService.GetAllUserMail();
            return ValidationResult.Success;
        }


        public ValidationResult GetAUser(string UserID, string OwnerID, out SEC_User obUser)
        {
            obUser = userDataService.GetAUser(UserID, OwnerID, out errorNumber);
            if (errorNumber.Length > 0)
            {
                return new ValidationResult(errorNumber, localizationService.GetResource(errorNumber));
            }
