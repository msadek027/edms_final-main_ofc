...
namespace SILDMS.Service.Owner
{
    public class OwnerService : IOwnerService
    {
        ...
        public ValidationResult GetOwnerWithPolicy(string ownerLavelID,string userID,out List<DSM_Owner> ownersList)
        {
            ownersList = _ownerDataService.GetOwnerWithPolicy(ownerLavelID,userID, out _errorNumber);
            if (_errorNumber.Length > 0)
            {
                return new ValidationResult(_errorNumber, _localizationService.GetResource(_errorNumber));
            }
