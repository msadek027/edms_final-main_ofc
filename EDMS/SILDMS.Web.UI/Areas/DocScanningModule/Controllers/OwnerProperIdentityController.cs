...
namespace SILDMS.Web.UI.Areas.DocScanningModule.Controllers
{
    public class OwnerProperIdentityController : Controller
    {
        ...
        public OwnerProperIdentityController(IOwnerProperIdentityService ownerProperIdentityRepository,
            ...
            IDocPropertyService docPropertyService, ILocalizationService localizationService, IAutoValueSetupService autoValueSetupService)
        {
        ...
        [HttpGet]
        [Authorize]
        public async Task<dynamic> GetOwnerLevelNew(string _OwnerLevelID)
        {
            List<DSM_OwnerLevel> result = null;
            await Task.Run(() => _ownerLevelService.GetOwnerLevel("", UserID, out result));
            return Json(new { Msg = "", result }, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            List<DSM_Owner> result = null;

            await Task.Run(() => _ownerService.GetAllOwners(_OwnerLevelID, UserID, out result));
        ...
        [HttpPost]
        [Authorize]
        public async Task<dynamic> GetOwner(string _OwnerLevelID)
        {
        ...
        [HttpPost]
        [Authorize]
        public async Task<dynamic> GetOwnerForOwnerLavelPolicy(string _OwnerLevelID)
        {
            List<DSM_Owner> objDsmOwner = null;

            await Task.Run(() => _ownerService.GetAllOwners(_OwnerLevelID, UserID, out objDsmOwner));
        ...
        {
            List<DSM_DocCategory> result = null;

            await Task.Run(() => _docCategoryService.GetDocCategories(_OwnerID, UserID, out result));
        ...
        [Authorize]
        public async Task<dynamic> GetDocumentCategories(string _OwnerID)
        {
        ...
        {
            List<DSM_DocType> result = null;

            await Task.Run(() => _docTypeService.GetDocTypes(_DocCategoryID, UserID, out result));

            return Json(new { Msg = "", result }, JsonRequestBehavior.AllowGet);
        }

        [Authorize]
        public async Task<dynamic> GetDocumentType(string _DocCategoryID, string _OwnerID)
        {
            ...
            var result = (from ow in objDsmDocTypes
                          where ow.Status == 1
                          //& ow.DocCategoryID == _DocCategoryID & ow.OwnerID == _OwnerID
             ...
             string _DocTypeID, string _DocPropertyID)
        {
            ...
            var result = (from dp in objDocPropIdentifies
                          ...
                          select new DSM_DocPropIdentify
                          {
                              ...
                              IsObsoleteIdentity=dp.IsObsoleteIdentity,
                              Remarks = dp.Remarks,
                              Status = dp.Status
                          }).ToList();
       ...
       // [SILLogAttribute]
        public async Task<dynamic> AddOwnerPropIdentity(DSM_DocPropIdentify modelDsmDocPropIdentify)
        {
            if (ModelState.IsValid)
            {
                ...
                modelDsmDocPropIdentify.ConfigureColumnIds = _autoValueSetupService.GetConfigureColumnList(System.Web.HttpContext.Current.Request.Url.AbsolutePath.ToString(), modelDsmDocPropIdentify.Owner.OwnerID,
                    modelDsmDocPropIdentify.DocCategory.DocCategoryID,modelDsmDocPropIdentify.DocType.DocTypeID, 
                    modelDsmDocPropIdentify.DocProperty.DocPropertyID,modelDsmDocPropIdentify.DocPropIdentifyID);
                
                respStatus = await Task.Run(() => _ownerProperIdentityService.AddOwnerPropIdentity(modelDsmDocPropIdentify, action, out outStatus));
                ViewBag.LoggID = "";
                ViewBag.LoggResult = "";
                ViewBag.LoggAction = "Insert DocPropIdentify";
                ViewBag.LookupTable = "DSM_DocPropIdentify";
        ...
        public async Task<dynamic> EditOwnerPropIdentity(DSM_DocPropIdentify modelDsmDocPropIdentify)
        {
            if (ModelState.IsValid)
            {
                ...
                ViewBag.LoggID = modelDsmDocPropIdentify.DocPropIdentifyID;
                ViewBag.LoggResult = "";
                ViewBag.LoggAction = "Edit DocPropIdentify";
                ViewBag.LookupTable = "DSM_DocPropIdentify";
