...
namespace SILDMS.Web.UI.Areas.DocScanningModule.Controllers
{
    public class DocCategoryController : Controller
    {
        ...
        #region Constructor
        public DocCategoryController( IAutoValueSetupService autoValueSetupService,IDocCategoryService docCategoryService, ILocalizationService localizationService, IOwnerService ownerService, IOwnerLevelService ownerLevelService)
        {
        ...
        public async Task<dynamic> AddDocCategory(DSM_DocCategory objDocCategory)
        {
            if (ModelState.IsValid)
            {
                ...
                objDocCategory.ConfigureColumnIds = _autoValueSetupService.GetConfigureColumnList("/DocScanningModule/DocCategory",
                    objDocCategory.OwnerID,objDocCategory.DocCategoryID,null,null,null);
                _respStatus = await Task.Run(() => _docCategoryService.ManipulateDocCategory(objDocCategory, _action, out _outStatus));
                // Error handling.   
                ViewBag.LoggID = objDocCategory.DocCategoryID;
                ViewBag.LoggResult = "";
                ViewBag.LoggAction = "Insert DocCategory";
                ViewBag.LookupTable = "DSM_DocCategory";
        ...
        public async Task<dynamic> EditDocCategory(DSM_DocCategory objDocCategory)
        {
            if (ModelState.IsValid)
            {
                ...
                ViewBag.LoggID = objDocCategory.DocCategoryID;
                ViewBag.LoggResult = "";
                ViewBag.LoggAction = "Edit DocCategory";
                ViewBag.LookupTable = "DSM_DocCategory";
                return Json(new { Message = _respStatus.Message, _respStatus }, JsonRequestBehavior.AllowGet);
            }
            else
        ...
        public async Task<dynamic> DeleteDocCategory(DSM_DocCategory objDocCategory)
        {
            ...
            _respStatus = await Task.Run(() => _docCategoryService.ManipulateDocCategory(objDocCategory, _action, out _outStatus));
            ViewBag.LoggID = objDocCategory.DocCategoryID;
            ViewBag.LoggResult = JsonConvert.SerializeObject(objDocCategory).ToString();
            ViewBag.LoggAction = "Delete";
            ViewBag.LookupTable = "DSM_DocCategory";
            return Json(new { Message = _respStatus.Message, _respStatus }, JsonRequestBehavior.AllowGet);
        }
