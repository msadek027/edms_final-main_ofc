...
namespace SILDMS.Web.UI.Areas.SecurityModule.Controllers
{
    public class OwnerLevelPermissionController : Controller
    {
        ...
        public OwnerLevelPermissionController(IOwnerLevelPermissionService ownerLevelPermissionService, IUserService userService, IOwnerService ownerService, ILocalizationService localizationService)
        {
        ...
        public async Task<dynamic> SetOwnerLevelPermission(SEC_UserOwnerAccess model)
        {
            ...
            {
                respStatus = await Task.Run(() => _OwnerLevelPermissionService.SetOwnerLevelPermission(model, out outStatus));

                List<UserWisePermittedOwner> userWisePermittedOwners = null;

                await Task.Run(() => _OwnerLevelPermissionService.GetUserWisePermittedOwnerList(model.UserID, UserID, model.OwnerLevelAccessID, out userWisePermittedOwners));
                ViewBag.LoggID = model.UserOwnerAccessID;
                ViewBag.LoggResult = "";
                ViewBag.LoggAction = "Edit UserOwnerAccess";
                ViewBag.LookupTable = "SEC_UserOwnerAccess";
        ...
        [Authorize]
        public async Task<dynamic> GetOwnersForOwnerLevelPermission(string _UserID, string _OwnerLevelID)
        {
            List<UserWisePermittedOwner> userWisePermittedOwners = null;
            ...
            var distinctResult = userWisePermittedOwners
