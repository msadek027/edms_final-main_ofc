...
namespace SILDMS.Web.UI.Areas.SecurityModule.Controllers
{
    public class RoleSetupController : Controller
    {
        ...
        public RoleSetupController(IRoleSetupService roleService, ILocalizationService localizationService )
        {
        ...
        public async Task<dynamic> GetRole(string _OwnerID)
        {
            ...
            var result = (from r in rolesList
                          ...
                          select new SEC_Role
                          {
                              ...
                              Status = r.Status,
                              PassResetInDays=r.PassResetInDays
                          }).ToList();
        ...
        public async Task<dynamic> EditRole(SEC_Role modelRole)
        {
            if (ModelState.IsValid)
            {
                ...
                ViewBag.LoggID = modelRole.RoleID;
                ViewBag.LoggResult = JsonConvert.SerializeObject(modelRole); ;
                ViewBag.LoggAction = "Edit Role";
                ViewBag.LookupTable = "SEC_Role";
                return Json(new { Message = respStatus.Message, respStatus }, JsonRequestBehavior.AllowGet);
            }
            else
            {
