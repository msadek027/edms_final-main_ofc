                ...
                else
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        docCategoriesList = dt1.AsEnumerable().Select(reader => new DSM_DocCategory
                        {
                            ...
                            InfoCopy = reader.GetBoolean("InfoCopy"),
                            AddText = reader.GetBoolean("AddText"),
        ...
        public string ManipulateDocCategory(DSM_DocCategory category, string action, out string errorNumber)
        {
            ...
            try
            {
                ...
                using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("SetDocCategory"))
                {
                    ...
                    db.AddInParameter(dbCommandWrapper, "@InfoCopy", SqlDbType.Int, category.InfoCopy ? 1 : 0);
                    db.AddInParameter(dbCommandWrapper, "@AddText", SqlDbType.Int, category.AddText ? 1 : 0);
                    db.AddInParameter(dbCommandWrapper, "@Action", SqlDbType.VarChar, action);
                    db.AddInParameter(dbCommandWrapper, "@ConfColumnIds", SqlDbType.VarChar, category.ConfigureColumnIds);
