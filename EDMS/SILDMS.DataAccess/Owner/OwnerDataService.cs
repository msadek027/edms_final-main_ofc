...
namespace SILDMS.DataAccess.Owner
{
    public class OwnerDataService : IOwnerDataService
    {
        ...
        public List<DSM_Owner> GetOwnerWithPolicy(string ownerLavelID, string userID, out string errorNumber)
        {
            errorNumber = string.Empty;
            List<DSM_Owner> ownersList = new List<DSM_Owner>();
            DatabaseProviderFactory factory = new DatabaseProviderFactory();
            ...
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("GetOwnerWithPolicy"))
            {
                db.AddInParameter(dbCommandWrapper, "@OwnerLavelId", SqlDbType.VarChar, ownerLavelID);
        ...
        public string ManipulateOwner(DSM_Owner owner, string action, out string errorNumber)
        {
            ...
            try
            {
                ...
                using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("SetOwner"))
                {
                    ...
                    db.AddInParameter(dbCommandWrapper, "@UDOwnerCode", SqlDbType.NVarChar, DataValidation.TrimmedOrDefault(owner.UDOwnerCode));
                    db.AddInParameter(dbCommandWrapper, "@OwnerLevelID", SqlDbType.NVarChar, owner.OwnerLevelID);
                    db.AddInParameter(dbCommandWrapper, "@OwnerName", SqlDbType.NVarChar, DataValidation.TrimmedOrDefault(owner.OwnerName));
                    db.AddInParameter(dbCommandWrapper, "@OwnerShortName", SqlDbType.NVarChar, DataValidation.TrimmedOrDefault(owner.OwnerShortName));
