...
namespace SILDMS.DataAccess.Users
{
    public class UserDataService : IUserDataService
    {
        ...
        public string AddUser(SEC_User objUser, string action, out string errorNumber)
        {
            ...
            try
            {
                ...
                using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("SetUsers"))
                {
                    ...
                    db.AddInParameter(dbCommandWrapper, "@obsulate", SqlDbType.Int, objUser.obsulate ? 1 : 0);
                ...
                if (!db.GetParameterValue(dbCommandWrapper, "@p_Status").IsNullOrZero())
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0 && (ds.Tables[1].Rows.Count > 0))
                    {
                        ...
                        accessList = dt1.AsEnumerable().Select(reader => new GetUserAccessPermission_Result
                        {
                            ...
                            DefaultServer = reader.GetString("DefaultServer"),
                            PasswordResetDate = reader.GetDateTime("PasswordResetDate"),
        ...
        public IpValidationInfo GetIpValidationInfo(string user, string ip)
        {
            DatabaseProviderFactory factory = new DatabaseProviderFactory();
            SqlDatabase db = factory.CreateDefault() as SqlDatabase;
            IpValidationInfo ipinfo = new IpValidationInfo();
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("CheckIpValidity"))
            {
                // Set parameters 
                db.AddInParameter(dbCommandWrapper, "@user", SqlDbType.NVarChar, user.Trim());
                db.AddInParameter(dbCommandWrapper, "@userIP", SqlDbType.NVarChar, ip);
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    var a = Convert.ToInt32(ds.Tables[0].Rows[0]["AttemptCount"]);
                    var b = Convert.ToInt64(ds.Tables[0].Rows[0]["LastAttempt"]);
                    if (Convert.ToInt32(ds.Tables[0].Rows[0]["AttemptCount"]) >= 3 && Convert.ToInt64(ds.Tables[0].Rows[0]["LastAttempt"]) <= 5 )
                    {
                        ipinfo.IsIpValid = false;
                        ipinfo.RemainSecond = Convert.ToInt64(ds.Tables[0].Rows[0]["LastAttempt"]);
                    }
                    else
                        ipinfo.IsIpValid = true;
        ...
        public List<string> GetAllUserMail()
        {
            ...
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("GetAllUserMail"))
            {
                // Set parameters 
             //   db.AddInParameter(dbCommandWrapper, "@user", SqlDbType.NVarChar, userid.Trim());

                DataSet ds = db.ExecuteDataSet(dbCommandWrapper);
                if (ds.Tables[0].Rows.Count > 0)
                { 
                    DataTable dt1 = ds.Tables[0];
                    mailList = dt1.AsEnumerable().Select(o => o.GetString("IntMailAddress")).ToList();              
                }
            }
            return mailList;
        }
        ...
        public SEC_User GetAUser(string UserID, string OwnerID, out string errorNumber)
        {
            errorNumber = string.Empty;
            ...
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("GetUserDetails"))
            {
                ...
                else
                {
                    ...
                    {
                        DataTable dt1 = ds.Tables[0];
                        user = dt1.AsEnumerable().Select(reader => new SEC_User
                        {
                            UserID = reader.GetString("UserID"),
                            OwnerLevelID = reader.GetString("OwnerLevelID"),
                            LevelName = reader.GetString("LevelName"),
