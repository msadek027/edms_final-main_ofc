...
namespace SILDMS.DataAccess.RoleSetup
{
    public class RoleSetupDataService: IRoleSetupDataService
    {
                ...
                else
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        roleList = dt1.AsEnumerable().Select(reader => new SEC_Role
                        {
                            ...
                            PassResetInDays=reader.GetInt32("PassResetInDays"),
        ...
        public string AddRole(SEC_Role modelRole, string action, out string errorNumber)
        {
            ...
            try
            {
                ...
                using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("SetRole"))
                {
                    ...
                    db.AddInParameter(dbCommandWrapper, "@PassResetInDays", SqlDbType.Int, modelRole.PassResetInDays);
        ...
        public SEC_Role GetRoleByRoleId( string RoleId)
        {
            ...
            try
            {
                DatabaseProviderFactory factory = new DatabaseProviderFactory();
                SqlDatabase db = factory.CreateDefault() as SqlDatabase;
                using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("GetRoleByRoleId"))
                {
                    db.AddInParameter(dbCommandWrapper, "@RoleID", SqlDbType.NVarChar, RoleId);


                    IDataReader dr=db.ExecuteReader(dbCommandWrapper);
                    while (dr.Read())
                    {
                        role.PassResetInDays = dr.GetInt32("PassResetInDays");
                    }
